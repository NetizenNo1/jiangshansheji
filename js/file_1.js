...
function 更新资料(){
	...
	if(id('tiliT')){id('tiliT').style.width =  Math.ceil(属性.体力 / 属性.体力上限 * 171) + 'px';}
 }
function 小提示(文本){
...
function 等待的大黑布(){
 	...
 	setTimeout(function() {id('dengdaihmb').style.display = 'none';var mc = 'images/3.webp';}, 1000);
}
function 背景模糊(){
...
function 新后宫图形读取程序(路径,头像,特殊){
	...
	if(路径 == 剧情头像){头像幕布();}
	if(路径 !== undefined){路径.src = _loc4_ + "Tu_" + 头像 + ".webp";}
	if(特殊 == "特殊"&&头像==0&&路径 !== undefined){路径.src =LJ + "a0aPic_TeShu/Tu_0.webp";}
	_loc1_= _loc4_ + "Tu_" + 头像 + ".webp";
	if(特殊 == "皇帝"){路径.src = LJ + "a0aPic_HuangDi/Tu_" + 主角.头像 + ".webp";}
	return _loc1_;
 }
...
function 生成外国女子(取数据, 姓氏){
   ...
   _loc7_ = _loc6_[1];
   _loc8_ = _loc6_[0];
   _loc9_ = 取数据.id;   _loc14_ = 60 + random(40);
