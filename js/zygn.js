	...
	id('seHuangdiOver').onclick = function (){
		...
		id('taijianSh').style.display = 'block';
			...
			id('taijianCon').onclick = function (){
				...
				_all_feizi[0].全名 = _all_feizi[0].家族 + _all_feizi[0].名字;
				_all_child.push({id:0,_code:8900,序号:0,名称:"少禛",性别:0,母亲:_all_feizi[0].名字,生母:"朱柔则",文化:8,年龄:2,婚:0,婚配:"",病:8000,保:9000,关系:10,幼年:200,头像:1200,统帅:3,武力:1,智力:17,政治:16,魅力:18,清廉:99,野心:70,功勋:0,寿命:999,被过继过:1,封号:"",文质:99,武质:33,魅质:90,朝廷势力:0,后宫势力:0,性情:12,状态:0});
...
function 皇宫的各个按钮(){
	...
	id('具体属性').onclick = function (){id('shuxingban').style.display = 'block';}
	var 皇宫 = id('皇宫');
	var 皇宫背景 = id('皇宫背景');
	var 宣政殿 = id('宣政殿');
	...
	id('寿康宫').onclick = function (){
		if(主角.当前时辰 == 0){太监说话的程序(4,"启禀皇上：\n太妃们清晨不喜被打扰。");}
	    	...
	    	id('jiajuanguanbi').onclick = function(){
...
function 世界地图(){
	...
	while(i < 47){
        ...
        id('cityxxbg').src = LJ+'azcPic_Jyou/' + this.取.id + '.webp';
...
function 椒房殿程序(){
	...
	id('san1').onclick = function(){
		...
		var 聊天内容 = [
			...
			"听闻皇上近日操劳国事，臣妾心中甚是心疼。",
			"皇上若有烦心事，不妨与臣妾说说。",
			"臣妾愿为皇上分忧解难，哪怕只是陪皇上聊聊天。",
			"后宫姐妹们都很好，皇上不必太过挂念。",
			"皇上今日来椒房殿，臣妾心中甚是欢喜。",
			"臣妾会好好管理后宫，不让皇上操心。"
		];
		var 随机对话 = 聊天内容[random(聊天内容.length)];
		对话打字效果(皇后.封号 + 皇后.位分 + 皇后.名字 + "：" + 随机对话);
		剧情对话板.onclick = function(){
			对话打字效果("（是否继续与皇后聊天？）", ["继续聊天", "离开"]);
			id('剧情an0').onclick = function(){
				出现这个人的头像(剧情头像,皇后);
				var 新对话 = 聊天内容[random(聊天内容.length)];
				对话打字效果(皇后.封号 + 皇后.位分 + 皇后.名字 + "：" + 新对话);
				剧情对话板.onclick = function(){
					对话打字效果("（是否继续与皇后聊天？）", ["继续聊天", "离开"]);
					id('剧情an0').onclick = function(){
						出现这个人的头像(剧情头像,皇后);
						var 再次对话 = 聊天内容[random(聊天内容.length)];
						对话打字效果(皇后.封号 + 皇后.位分 + 皇后.名字 + "：" + 再次对话);
						剧情对话板.onclick = function(){
							对话打字效果("（皇后似乎还有很多话想对皇上说。）", ["继续聊天", "离开"]);
							id('剧情an0').onclick = function(){
								出现这个人的头像(剧情头像,皇后);
								对话打字效果(皇后.封号 + 皇后.位分 + 皇后.名字 + "：皇上，臣妾还有很多话想对您说呢...");
								剧情对话板.onclick = function(){
...
function 更新宫信息(参数,对象,临时宫殿列表,序列){
	...
	while(i < 5){
		...
		if(_loc1_ == undefined){
		    ...
		    className('选宫背景')[i].src =LJ + 'a0aPic_Placae/Tu_' + _loc1_.跳 + '.webp';
...
function 宫殿内部(参数,对象,宫){
	...
	id('宫背景').src =LJ +  'azcPic_Gong_Biao/' + 宫.跳 + '.webp';
	if(参数 !== 0 &&参数 !== 62){
		var 主位条件 = 1;
	    f = 0;
	    while(f < 5){
...
function 学习事件处理(参数){
    ...
    if(参数 == 3){
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_4.webp";
      属性.体力 -= 40;
      ...
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_2.webp";
      ...
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_3.webp";
      属性.体力 -= 40;
      ...
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_5.webp";
      属性.体力 -= 30;
      id('yyText').innerText = "前往慈宁宫向太后请安。道德上升。";
      属性.道德 += random(4) + 3;
      var _loc5_ = random(5);
      if(年 <= 2 && 国家.事件三 == 0){
      ...
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_87245.webp";
      属性.体力 -= 40;
      ...
      id('yyBG').src =LJ +  "a0aPic_BG/Tu_413248.webp";
      属性.体力 -= 30;
